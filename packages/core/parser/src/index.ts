...
function formatError(message: string, input: string, line?: number, column?: number, expected?: string, found?: string): string {
  let errorMsg = '';
...
function buildNestedStructure(elementsWithIndent: any[], input: string, lineMap: number[]): Element[] {
...
export function parse(input: string): Document {
  ...
  try {
    ...
    if (e.location && e.location.start) {
      ...
      const expected = e.expected ? e.expected.map((exp: any) => exp.description || exp.text || String(exp)).join(', ') : undefined;
